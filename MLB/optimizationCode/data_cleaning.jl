...
function read_player_data(path_hitters,path_pitchers)
    ...
    Team = [pitchers[:teamAbbrev]; hitters[:teamAbbrev]];
    Opp=[pitchers[:Opponent]; hitters[:Opponent]];
    Game = [];
    for i = 1:size(Team)[1]
        t1 = clean_str(Team[i]);
        t2 = clean_str( Opp[i]);
    ...
    proj_dfn=map(clean_num, [pitchers[:Projection_dfn]; hitters[:Projection_dfn]]);
    proj_bbm=map(clean_num, [pitchers[:Projection_baseballmonster]; hitters[:Projection_baseballmonster]]);
    proj_rotogrinders = map(clean_num, [pitchers[:Projection]; hitters[:Projection]]);
    proj_rotowire = map(clean_num, [pitchers[:Projection_rotowire]; hitters[:Projection_rotowire]]);
    players = DataFrame(Name = [pitchers[:Name]; hitters[:Name]],
                        teamAbbrev = [pitchers[:teamAbbrev]; hitters[:teamAbbrev]],
                        Opponent=[pitchers[:Opponent] ;hitters[:Opponent]],
                        Game = Game,
                        Position=[pitchers[:Position] ;hitters[:Position]],
        ...
        for i =1:num_players
            if tracer[i,j] == 1
                ...
                if P[i]==1  
                    ...
                    if lineup[P_index]==""
                        lineup[P_index] = string(players[i,:Name])
                    elseif lineup[P_index+1] ==""
                        lineup[P_index+1] = string(players[i,:Name])
                    else
                ...
                elseif C[i]==1
                    if lineup[C_index]==""
                        lineup[C_index] = string(players[i,:Name])
                    else
                        println(string("error in lineup",j,"player",i))
                    end
                elseif oneB[i]==1
                    if lineup[oneB_index]==""
                        lineup[oneB_index] = string(players[i,:Name])
                    else
                        println(string("error in lineup",j,"player",i))
                    end
                elseif twoB[i]==1
                    ...
                    if lineup[twoB_index]==""
                        lineup[twoB_index] = string(players[i,:Name])
                    else
                        println(string("error in lineup",j,"player",i))
                    end
                elseif threeB[i]==1
                    ...
                    if lineup[threeB_index]==""
                        lineup[threeB_index] = string(players[i,:Name])
                    else
                        println(string("error in lineup",j,"player",i))
                    end
                elseif SS[i]==1
                    ...
                    if lineup[SS_index]==""
                        lineup[SS_index] = string(players[i,:Name])
                    else
                        println(string("error in lineup",j,"player",i))
                    end
                else
                    ...
                    if lineup[OF_index]==""
                        lineup[OF_index] = string(players[i,:Name])
                    elseif lineup[OF_index+1] ==""
                        lineup[OF_index+1] = string(players[i,:Name])
                    elseif lineup[OF_index+2] ==""
                        lineup[OF_index+2] = string(players[i,:Name])
                    else
                        println(string("error in lineup",j,"player",i))
                    end
...
function create_lineups(num_lineups, num_overlap, stack_size,formulation, path_pitchers, path_hitters, path_covar_matrix, covar_lambda, exposure, exposure_P,exposure_B1,exposure_B2,exposure_B3,exposure_C,exposure_SS,exposure_OF, min_pitcher_exposure, path_to_output)
    ...
    for i =1:num_players
        ...
        elseif contains(pos,"OF") 
            ...
            println("\t",players[i,:Name]," has no position\n")
    ...
    for j=1:size(teams)[1]
        if players[1, :teamAbbrev] == teams[j]
            player_info[j] =1
        end
    end
    ...
    for i=2:num_players
        player_info = zeros(Int, num_teams)
        for j=1:size(teams)[1]
            if players[i, :teamAbbrev] == teams[j]
                player_info[j] =1
            end
        end
    ...
    # Populate player_info with the corresponding information
    for j=1:num_teams
        if players[1, :Opponent]=='@'
            opp = players[1, :Opponent][2:end]
        else
            opp = players[1, :Opponent]
        end
    ...
    for i=2:num_players
        ...
        for j=1:size(teams)[1]
            if players[i, :Opponent]=='@'
                opp = players[i, :Opponent][2:end]
            else
                opp = players[i, :Opponent]
            end
            if opp == teams[j]
                player_info[j] =1
            end
