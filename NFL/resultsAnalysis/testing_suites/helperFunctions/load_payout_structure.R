####### DESCRIPTION #########
# This file loads the full Payout Structure into R
# Special notes for contest.entry.fee: for week 9, use $4 in lieu of $3 and for week 10, $27 in lieu of $20
# If the file does not exist, returns 0


load_payout_structure_results <- function(week_number, contest_entry_fee, slate, year) {
  if (slate == 'main_slate') {
    if (contest_entry_fee == '$3') {
      if (week_number != 10) {
        file_name <- paste0("resultsAnalysis/data_warehouse/weekly_payout_structure/", year, "/", slate, "/",
                            contest_entry_fee, "_payout_structure_week", week_number, 
                            ".csv")
      } else{
        file_name <- paste0("resultsAnalysis/data_warehouse/weekly_payout_structure/", year, "/", slate, "/",
                            '$4', "_payout_structure_week", week_number, ".csv")
      }
      # Milly Maker / $20 Contests
    } else { 
      if (week_number > 9 && week_number != 17) {
        file_name <- paste0("resultsAnalysis/data_warehouse/weekly_payout_structure/", year, "/", slate, "/",
                            '$27', "_payout_structure_week", week_number, ".csv")
      } else {
        file_name <- paste0("resultsAnalysis/data_warehouse/weekly_payout_structure/", year, "/", slate, "/",
                            contest_entry_fee, "_payout_structure_week", week_number, ".csv")
      }
    }
