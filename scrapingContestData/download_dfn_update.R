...
download_dfn_update <- function(update_date) {
  original_wd <- getwd()
  ...
  dfn_projections_path <- file.path(original_wd,'MLB/data_warehouse/projections/dailyfantasynerd/updates')
  
  setwd(dfn_projections_path)
  pitchers_file_name <- paste0('pitchers_', update_date, ".csv")
  hitters_file_name <- paste0('hitters_', update_date, ".csv")
