...
download_DK_daily_contests_NBA <- function(contest_info, days_ahead) { 
  original_wd <- getwd()
  ...
  url <- paste0('https://www.draftkings.com/lobby#/NBA/0/Tournament')
  ...
  df <- df[, vars_needed]
  df$Contest_Date <- Sys.Date()+days_ahead
  ...
  # remove contests with less than 1000 entries
  df <- df[as.numeric(df$Contest_Size) >= 1000,]
  ...
  # only keep rows where slate day matches date day
  df <- df[grepl(substr(weekdays(as.Date(Sys.Date()+days_ahead,'%d-%m-%Y')),1,3), df$Slate),]
  
  return_df <- rbind(contest_info, df)
  write.csv(return_df, file = 'NBA/data_warehouse/contests.csv', row.names = F)
  return(return_df)
}
