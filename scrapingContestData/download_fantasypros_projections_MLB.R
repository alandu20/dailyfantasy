...
download_fantasypros_projections_MLB <- function() {
  original_wd <- getwd()
  ...
  url <- paste0('https://secure.fantasypros.com/accounts/login/?next=http://www.fantasypros.com/index.php')
  mybrowser$navigate(url)
  
  
  username = mybrowser$findElement(using = "name", "username")
  username$sendKeysToElement(list('dfsoptimizers'))
  password = mybrowser$findElement(using = "name", "password")
  password$sendKeysToElement(list('pizza1995', key = "enter"))
  Sys.sleep(2)
  mybrowser$navigate('https://www.fantasypros.com/mlb/projections/daily-hitters.php')
  Sys.sleep(2)
  
  # Download Hitters
  download_hitters_button = mybrowser$findElement(using = 'xpath', value = '//*[contains(concat( " ", @class, " " ), concat( " ", "fa-fp-download", " " ))]')
  download_hitters_button$clickElement()
  
  #Switch to Pitchers
  mybrowser$navigate('https://www.fantasypros.com/mlb/projections/daily-pitchers.php')
  Sys.sleep(2)
  
  # Download Pitchers
  download_pitchers_button = mybrowser$findElement(using = 'xpath', value = '//*[contains(concat( " ", @class, " " ), concat( " ", "fa-fp-download", " " ))]')
  ...
  setwd('~/Downloads')
  pitchers_file_name <- 'FantasyPros_2017_Projections_P'
  hitters_file_name <- 'FantasyPros_2017_Projections_H'
  
  while(!file.exists(pitchers_file_name)){
    Sys.sleep(1)
  }
  
  fantasypros_hitters <- read.csv(hitters_file_name, stringsAsFactors = F)
  fantasypros_pitchers <- read.csv(pitchers_file_name, stringsAsFactors = F)
  
  file.remove(hitters_file_name)
  file.remove(pitchers_file_name)
  #setwd(paste0(original_wd, 'MLB/data_warehouse/', date)
  dfn_projections_path <- file.path(original_wd,'MLB/data_warehouse/projections/fantasypros')
  
  setwd(bbmonster_projections_path)
  pitchers_file_name <- paste0(pitchers_file_name,'_', Sys.Date(), ".csv")
  hitters_file_name <- paste0(hitters_file_name, '_', Sys.Date(), ".csv")
  write.csv(fantasypros_pitchers, file = pitchers_file_name, row.names = F)
  write.csv(fantasypros_hitters, file = hitters_file_name, row.names = F)
